...
def train_test_split(
    ...
    X_train, X_test, y_train, y_test = sklearn_train_test_split(X, y, test_size=test_ratio,stratify=y, random_state=42)
...
def get_data_batch(
    ...
    split = StratifiedShuffleSplit(n_splits=1, test_size=batch_size / len(X), random_state=42)
    for _, idxs in split.split(X, y):
...
def plot_tsne(
    ...
    z2 = tsne.fit_transform(z.detach().numpy())
