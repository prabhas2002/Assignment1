...
def calculate_loss(
    ...
    epsilon = 1e-7  #for log(0) and log(1) case
    if is_binary:
        ...
        y_preds = y_preds[np.arange(len(y_preds)), y]    # get the predicted probability of the true class
        loss = -np.mean(np.log(y_preds+epsilon))         # cross-entropy loss
...
def fit_model(
    ...
    for i in range(num_iters + 1):
        ...
        if is_binary:
            ...
            y_batch_one_hot = np.eye(model.out_dim)[y_batch]
            
            grad_W = (X_batch.T @ (y_preds - y_batch_one_hot))/len(y_batch)
            grad_b = np.mean(y_preds - y_batch_one_hot, axis=0)
            
        
        
        # regularization
        # grad_W += l2_lambda * np.log(model.W ** 2)
        # grad_b += l2_lambda * np.log(model.b ** 2)
        
        # clip gradient norm
        #raise NotImplementedError('Clip gradient norm here')
        # norm = np.sqrt(np.sum(grad_W ** 2) + np.sum(grad_b ** 2))
        # if norm > grad_norm_clip:
        #     grad_W = (grad_norm_clip / norm) * grad_W
        #     grad_b = (grad_norm_clip / norm) * grad_b
        #loss not converging with this code
